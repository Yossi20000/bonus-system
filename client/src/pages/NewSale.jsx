          ...
          {results.map(p => (
            <div key={p.id} className="product-result-item" onClick={() => select(p)}>
              ...
              <div className="product-result-price">
                ...
                {p.bonus_type === 'fixed' ? `בונוס: ₪${p.bonus_value}` : `בונוס: ${p.bonus_value}%`}
              </div>
            </div>
          ))}
        </div>
...
function SaleItem({ item, onChange, onRemove }) {
  const calcBonus = () => {
    ...
    const val = parseFloat(item.product.bonus_value) || 0;
    if (item.product.bonus_type === 'fixed') return val;
    ...
    <div className="sale-item-row">
      <div>
        ...
        <div style={{ fontSize: '0.78rem', color: 'var(--text2)' }}>
          בונוס: {item.product.bonus_type === 'fixed' ? `₪${item.product.bonus_value} קבוע` : `${item.product.bonus_value}%`}
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <input
          type="number" value={item.sell_price}
...
export default function NewSale() {
  ...
  const totalBonus = items.reduce((sum, item) => {
    const price = parseFloat(item.sell_price) || 0;
    const val = item.product.bonus_value;
    const bonus = item.product.bonus_type === 'fixed' ? val : Math.round(price * val / 100 * 100) / 100;
    return sum + bonus * item.quantity;
  }, 0);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {user?.role === 'admin' && (
              <div className="form-group">
                <label>עובד (ברירת מחדל: אתה)</label>
