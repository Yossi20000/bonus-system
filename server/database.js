...
function createTables() {
  ...
  // Add overhead column if upgrading existing DB
  try { db.run('ALTER TABLE products ADD COLUMN overhead REAL DEFAULT 100'); } catch(e) {}
...
function seedInitialData() {
  ...
  // Always re-seed products to fix bonus calculations
  const cnt = get('SELECT COUNT(*) as c FROM products');
  if (!cnt || cnt.c === 0) {
    seedProducts();
  } else {
    // Update bonus_type and overhead for existing products to fix the calculation
    updateProductBonuses();
  }
}

function updateProductBonuses() {
  // Update all existing products to use profit_pct with correct cost/overhead
  const updates = getProductData();
  for (const p of updates) {
    const existing = get('SELECT id FROM products WHERE name = ?', [p.name]);
    if (existing) {
      run(`UPDATE products SET bonus_type=?, bonus_value=?, cost=?, overhead=?, sell_price=? WHERE name=?`,
        [p.bonus_type, p.bonus_value, p.cost, p.overhead, p.sell_price, p.name]);
    }
  }
  saveDb();
  console.log('Product bonuses updated');
}

function getProductData() {
  // bonus_type: 'profit_pct' = (sell/1.03 - overhead - cost) * pct%
  //             'sell_pct'   = sell * pct%   (מגנים)
  //             'none'       = 0 bonus
  return [
...
function seedProducts() {
  ...
  saveDb();
  console.log('Products seeded with correct bonus calculations');
}
